...
let lvl1 = {
    name : "182 Main St.",
    romm: " A mail box , 2 dead bodys , 2 Gremellin\n",
    romm2:" Big boos" ,
    mailBox: "Act1-Letter"
    }
...
async function appearMOb(){
    let gremAttackMin = 3 ;
        let gremAttackMax =  15 ;
        let trueAttack = Math.floor(Math.random(gremAttackMax, gremAttackMin)) +1;
        let mylife = 100;
    let maxNum = 3;
   ...
   if ( tentar == x){
     ...
     let life2 = mylife - trueAttack 
     console.log("You lose " + trueAttack + "!")
     console.log("Your Hp now is" + life2)
 appearMOb();
 }
  if (gremHP == 0){
      console.log("You defeat Gremelin!")
      let loot = await ask (">_")
      if( loot.includes("check") || loot.includes("open")){
          console.log(mob.Gremelin.iventory)
          lootMonster();
                                                          }
                                                          }
...
while(bigBoos.HP > 1 || bigBoos.HP <= 100 || playerLife > 1 || playerLife <= 100){
...
if (answer === bossAnswer){
    ...
    console.log(bigBoos.hp - 100);
let newlvl = await ask(">_");
if(newlvl.includes("12345")){
    startlvl2();
}
}
...
if( answer.includes("mail box")){
 ...
 else if(answer.includes("examine") || answer.includes("open")){
 ...
 else if(answer.includes("gremelin")){
